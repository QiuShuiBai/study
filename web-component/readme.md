...
如果写成一个 web components 的形式则需要首先在 html 中添加我们自定义的标签，在这里我们叫 `custom-dialog`。

```html
<custom-dialog></custom-dialog>
```

接下来，我们需要添加关于 `custom-dialog` 的定义：
...
> HTMLElement介绍 （可忽略这段）
>
> 我们先介绍下 `HTMLElement`，根据上面写法我们可以先认为这是一个类。所有的 html 元素都是基于这个类来的。
>
...
我们定义一个 `CustomDialog` 类并继承 `HTMLElement` 类。（当然我们可以继承其他标签类，其他标签类这块后续再提。）之后便在类中定义我们组件的行为等。（通过dom操作）

我们的 dialog 需要有标题、内容和一个按钮，那么我们先生成这三个元素：
...
定义完毕后，我们需要通过 `window.customElements.define` 这个 api 进行注册。整体如下：
...
现在整体上可以理解为，我们在 html 处添加了我们自定义的 `custom-dialog` 标签，然后通过继承 `HTMLElement` 基础类定义好了 `CustomDialog` 类，最后通过
`customElements.define` 注册了一个 web components 组件。

### 样式问题

现在一个最基本的内容就出来了，当然我们暂时还没有样式。让我们添加一下简单的样式文件。我们可以如正常写 css 一般给 web components 组件添加类名。
...
上面我们提到，定义完一个 web components 类后，需要调用 `customElements.define` 对其进行注册。
...
当我们定义类时继承了一个具体的标签类，那么在注册时，就需要传入 `define` 的第三个参数。下方代码选择了继承 input，且`define` 传入 extends 参数。
...
通过继承标签子类，可以省去我们手动去对一些属性进行传递，有点类似于 vue 语法中的 [v-bind="$attrs"](https://cn.vuejs.org/v2/api/#vm-attrs)。比如上面的 `custom-input` 如果要改变 type 类型，用户在使用时，直接更改 type 就好，而若是通过继承基础类 `HTMLElement`，那么我们需要人工 `createElement('input')` 一个 input，再获取到用户定义的 type 后手动更新。（关于获取，后续会说到）

定义完毕后，我们可以通过 `document.createElement('custom-input')` 创建标签，或者可以通过 `new CustomInput()` 进行创建
...
shadow DOM 主要为了隔离我们定义的组件与 dom，起到互不影响的作用。比如外部的 css 不影响到我们的组件。使用也很简单：
...
可以看到 mode 为 open 时，我们写在外部的 css 已经不能影响到 shadow dom 中的代码，那么如果有定义样式的需求时，该如果做呢？答案也很简单，既然我们能拿到 shadowRoot 结点，那么直接定义好 css 样式，在通过 js 添加进去就好。
...
当 mode 为 closed 时，则外部无法通过 shadowRoot 访问，此时我们外部就无法对组件内容进行操作了

<img src="https://dpubstatic.udache.com/static/dpubimg/80e308bf-5e7c-4189-9748-5ce9eb15d807.jpeg">

不论 attachShadow 中 mode 参数为 open 或 closed，我们的样式都只能写在自定义组件内，不能再像最开始一样写在全局。
...
slot 和我们 vue 中的 slot 很相似，这里简单提供一个例子。我们提供了一个具名插槽 `tips`
...
然后在使用处，传入我们的要插入的结点，并且选择 `tips` 插槽
...
可以看到我们的 tips 在dom结构里，写在了 shadow root 外，所以 tips 这个dom可以由用户自由操作
...
2. attributeChangedCallback 需要被监听的属性才会触发。需使用observedAttributes() get 函数进行配合
    ...
    这样我们的 content 和 tips 修改时，就会触发 `attributeChangedCallback`
